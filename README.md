<h1>Hi, I'm Himanshu! <br/>

<h2>Bioinformatics Projects:</h2>

<h2>Prognostic lncRNA Signature in High-Grade Serous Ovarian Cancer</h2>

| # | Project | Question | Result |
|---|---|---|---|
| 01 | [Differential expression]([01-differential-expression](https://github.com/himanshuguptaofficial/Bioinformatics-Developmental-Projects/tree/main/01-differential-expression)) | Which lncRNAs differ between resistant and sensitive patients? | 554 at raw p < 0.05, none survive FDR correction |
| 02 | [Survival risk score](02-survival-risk-score) | Does a composite of five lncRNAs stratify survival? | log-rank p = 1.2 × 10⁻⁹, roughly a 2-year median survival gap |
| 03 | [Time-dependent ROC](03-time-dependent-roc) | Does the lncRNA signature beat FIGO stage and age predictions of disease outcome? | 5-year AUC 0.748 against 0.480 (age) and 0.471 (stage) |
| 04 | [Pathway enrichment](04-pathway-enrichment) | Which biological pathways are associated with the lncRNA signature? | Overwhelmingly immune, cytokine and T-cell signaling |

<h2>  Connect with me:</h2>

[<img align="left" alt="Himanshu Gupta | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]

[linkedin]: www.linkedin.com/in/himanshu-gupta-973826288

<!--
**joshmadakor1/joshmadakor1** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
